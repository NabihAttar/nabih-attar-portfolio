...
async function loadFrame(frameIndex, cache) {
  if (cache[frameIndex]) return cache[frameIndex];

  const { webp, png } = getSonicwaveFramePaths(frameIndex);
...
async function preloadFramesProgressive(
  ...
  await loadFrame(loadOrder[0], images);
  ...
  for (let i = 1; i < loadOrder.length; i += batchSize) {
    ...
    await Promise.all(
      slice.map(async (frameIndex) => {
        await loadFrame(frameIndex, images);
...
export function SonicWaveScrollytellingHero() {
  ...
  const introOpacity = useTransform(scrollYProgress, [0, 0.12, 0.28], [1, 1, 0]);
  const introY = useTransform(scrollYProgress, [0, 0.28], [0, -48]);
  const hintOpacity = useTransform(scrollYProgress, [0, 0.08, 0.18], [1, 1, 0]);
  const titleOpacity = useTransform(scrollYProgress, [0.05, 0.22, 0.55, 0.72], [0, 1, 1, 0]);
  const titleY = useTransform(scrollYProgress, [0.05, 0.72], [32, -72]);
  ...
  }, [frameIndices, maxFrameIndex, requestRedraw]);
  ...
  return (
    <section
      ...
      style={{ height: `${SONICWAVE_SCROLL_HEIGHT_VH}vh` }}
      aria-label="SonicWave Pro cinematic product reveal"
    >
      <div
      ...
      >
        ...
        <motion.div className="pointer-events-none absolute inset-0 z-10 flex flex-col">
          <motion.div
            className="flex flex-1 flex-col items-start justify-center px-6 sm:px-12 lg:px-20"
            style={{ opacity: introOpacity, y: introY }}
          >
            <p className="mb-4 text-[11px] font-medium uppercase tracking-[0.4em] text-white/45">
              Introducing
            </p>
            <h2 className="max-w-xl text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:text-6xl">
              SonicWave Pro
            </h2>
            <p className="mt-4 max-w-md text-base text-white/55 sm:text-lg">
              Precision audio engineering, revealed frame by frame.
            </p>
          </motion.div>

          <motion.div
            className="absolute left-6 top-1/2 z-10 -translate-y-1/2 sm:left-12 lg:left-20"
            style={{ opacity: titleOpacity, y: titleY }}
          >
            <p className="text-[11px] font-medium uppercase tracking-[0.45em] text-cyan-300/80">
              SonicWave Pro
            </p>
            <h3 className="mt-3 text-3xl font-light tracking-tight text-white sm:text-4xl lg:text-5xl">
              Sound
          ...
          <motion.p
            className="absolute bottom-36 left-6 max-w-xs text-sm leading-relaxed text-white/40 sm:left-12 lg:left-20"
            style={{ opacity: subtitleOpacity, y: subtitleY }}
          >
            Scroll through the transformation — every layer tells the story of
          ...
          <motion.div
            className="pointer-events-auto absolute inset-x-0 bottom-0 flex justify-center px-6 pb-12 sm:pb-16"
            style={{ opacity: ctaOpacity, y: ctaY }}
          >
            <a
              href="#home"
              className="group inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/[0.06] px-8 py-4 text-sm font-medium text-white backdrop-blur-md transition-[transform,background-color,border-color] duration-300 hover:border-white/25 hover:bg-white/10"
            >
              <span>Experience SonicWave Pro</span>
